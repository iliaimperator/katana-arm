...
@section('content')
<div class="max-w-4xl">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Редактирование автомобиля</h1>

    <form action="{{ route('cars.update', $car->car_id) }}" method="POST" class="bg-white rounded-lg shadow p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <select name="client_id" id="client_id" required
                ...
                @foreach($clients as $client)
                    <option value="{{ $client->client_id }}" {{ old('client_id', $car->client_id) == $client->client_id ? 'selected' : '' }}>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="engine_model" id="engine_model"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    value="{{ old('engine_model', $car->engine_model) }}"
                    placeholder="1AZ-FE">
                @error('engine_model')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                <label for="engine_volume" class="block text-sm font-medium text-gray-700 mb-2">Объем двигателя (л):</label>
                <input type="number" step="0.1" name="engine_volume" id="engine_volume"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    value="{{ old('engine_volume', $car->engine_volume) }}"
                    placeholder="2.4" min="0.5" max="10.0">
                @error('engine_volume')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mb-6">
            ...
            <textarea name="notes" id="notes" rows="3"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder="Особенности автомобиля, предыдущие ремонты и т.д.">{{ old('notes', $car->notes) }}</textarea>
            @error('notes')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
